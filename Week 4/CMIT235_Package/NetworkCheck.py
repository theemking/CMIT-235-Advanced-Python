...
class NetworkCheck:
    ...
    #  create the following two overloaded functions:
    def __checkCounts(self, csv_data, *features):
        counts = {}
        ftc = pa.read_csv(csv_data, sep=',')
        for i in features:
            counts[i] = ftc[i].value_counts()
        return counts

    @dispatch(str, str)
    def checkCounts(self, csv_data, feature):
        ftc = pa.read_csv(csv_data, sep=",")
        return ftc[feature].value_counts()

    '''
      Similar to checkCounts with one feature, however instead of returning values_counts for one feature, 
      return a dictionary of key:value pairs for value_counts of feature1, 2, and 3.
    '''
